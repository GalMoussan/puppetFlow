...
2. **Spawn a background execution Agent** (`run_in_background: true`) with this prompt:
   ...
   Read and follow the full process in `.Codex/skills/company-execute/SKILL.md`.
   ...
   ```
   - Use the default `Codex` agent type (it can spawn its own sub-workers and use the test tools).
